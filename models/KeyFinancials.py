...
class KeyFinancials(object):
    ...
    def __init__(self, worksheet):
        ...
        cell_address = find_cell(worksheet, KeyFinancials.REV_CHOICES[0], row_offset=74)
        if cell_address:
            row_rev, col_rev, = cell_address
            self.gross_rev = get_object(worksheet, row_rev, col_rev)

        cell_address = find_cell(worksheet, KeyFinancials.REV_CHOICES[1], row_offset=74)
        if cell_address:
            row_rev, col_rev, = cell_address
            self.net_rev = get_object(worksheet, row_rev, col_rev)

        cell_address = find_cell(worksheet, KeyFinancials.REV_CHOICES[2], row_offset=74)
        if cell_address:
            row_rev, col_rev, = cell_address
            self.net_nii = get_object(worksheet, row_rev, col_rev)

        cell_address = find_cell(worksheet, KeyFinancials.REV_CHOICES[3], row_offset=74)
        if cell_address:
            row_rev, col_rev, = cell_address
        ...
        self.ebit = None
        self.ppop = None
        cell_address = find_cell(worksheet, KeyFinancials.EB_CHOICES[0], row_offset=74)
        if cell_address:
            row_ebit, col_ebit = cell_address
            self.adj_ebitda = get_object(worksheet, row_ebit, col_ebit)

        cell_address = find_cell(worksheet, KeyFinancials.EB_CHOICES[1], row_offset=74)
        if cell_address:
            row_ebit, col_ebit = cell_address
            self.ebitdar = get_object(worksheet, row_ebit, col_ebit)

        cell_address = find_cell(worksheet, KeyFinancials.EB_CHOICES[2], row_offset=74)
        if cell_address:
            row_ebit, col_ebit = cell_address
            self.ebita = get_object(worksheet, row_ebit, col_ebit)

        cell_address = find_cell(worksheet, KeyFinancials.EB_CHOICES[3], row_offset=74)
        if cell_address:
            row_ebit, col_ebit = cell_address
            self.ebit = get_object(worksheet, row_ebit, col_ebit)

        cell_address = find_cell(worksheet, KeyFinancials.EB_CHOICES[4], row_offset=74)
        if cell_address:
            row_ebit, col_ebit = cell_address
        ...
        cell_address = find_cell(worksheet, KeyFinancials.INCOME_CHOICES[0], row_offset=74)
        if cell_address:
            row_income, col_income = cell_address
            self.net_income_gaap = get_object(worksheet, row_income, col_income)

        cell_address = find_cell(worksheet, KeyFinancials.INCOME_CHOICES[1], row_offset=74)
        if cell_address:
            row_income, col_income = cell_address
        ...
        cell_address = find_cell(worksheet, KeyFinancials.FCF_CHOICES[0], row_offset=74)
        if cell_address:
            row_pcfcf, col_pcfcf = cell_address
            self.free_cash_flow = get_object(worksheet, row_pcfcf, col_pcfcf)

        cell_address = find_cell(worksheet, KeyFinancials.FCF_CHOICES[1], row_offset=74)
        if cell_address:
            row_pcfcf, col_pcfcf = cell_address
