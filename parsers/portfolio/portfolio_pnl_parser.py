    ...
    def export(self):
        ...
        for portfolio in portfolios:
            ...
            try:
                ...
                for item in ts:
                    if bool(item):
                        worksheet.write('A{}'.format(count), portfolio.analyst, merge_format)
                        worksheet.write('B{}'.format(count), item['pnl_date'].strftime('%m/%d/%y'), merge_format)
                        worksheet.write('C{}'.format(count), item['mtd_pct_return_long'], merge_format)
                        worksheet.write('D{}'.format(count), item['ytd_pct_return_long'], merge_format)
                        worksheet.write('E{}'.format(count), item['market_value_long'], merge_format)
                        worksheet.write('F{}'.format(count), item['last_price_long'], merge_format)
                        worksheet.write('G{}'.format(count), item['mtd_usd_return_long'], merge_format)
                        worksheet.write('H{}'.format(count), item['ytd_usd_return_long'], merge_format)
                        worksheet.write('I{}'.format(count), item['mtd_pct_return_short'], merge_format)
                        worksheet.write('J{}'.format(count), item['ytd_pct_return_short'], merge_format)
                        worksheet.write('K{}'.format(count), item['market_value_short'], merge_format)
                        worksheet.write('L{}'.format(count), item['last_price_short'], merge_format)
                        worksheet.write('M{}'.format(count), item['mtd_usd_return_short'], merge_format)
                        worksheet.write('N{}'.format(count), item['ytd_usd_return_short'], merge_format)
                        count += 1
